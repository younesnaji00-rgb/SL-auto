...
- **Step 1 arrangement** (NN/g: tabs only for content not needed simultaneously): tab **Informations** = purpose-named picker « Pré-remplir depuis un document » + one-line source status + the form, with the source document rendered in a side pane by default at ≥ lg (verification is a side-by-side task); tab **Pièces** = required-pieces list + « Ajouter des pièces » + the single primary « Envoyer vers chiffrage ». Both tabs carry a state badge (missing fields · n/m pièces) so nothing is hidden behind a label.
