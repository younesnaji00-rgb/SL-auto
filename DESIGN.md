...
- **Glass = the chrome layer only** (2026-09-01, user chose "chrome only" over the full Markazi port). Apple HIG Materials / Liquid Glass split: bars, sidebar, menus, popovers, selects, dialogs and sheets are frosted (`.glass-bar` for sticky bars content scrolls under — top bar, record bar, stepper strip, mobile bottom bar; `.glass-strong` for overlays; `.glass-sidebar` for the rail + drawer); the CONTENT layer — `.paper`, `Card`, sockets, forms, the context column — is never glass. Tokens `--glass-bg-strong/bar/sidebar`, `--glass-border`, `--glass-highlight` (inset top edge), `--glass-shadow-panel`, `--scrim` (dialog overlay, `bg-[color:var(--scrim)] backdrop-blur-[6px]`), `--grain-opacity`. No ambient mesh/gradient (user ruling 2026-09-01 — "remove the gradient"): the canvas is flat cream; the frost shows where content scrolls under the bars or sits behind overlays. `body::after` adds the grain (z 1, under bars/overlays). Nested-solid rule: `.glass-strong .glass-strong` flattens to `popover`. Fallbacks: `prefers-reduced-transparency`, no `backdrop-filter`, print → solid. Never add `.glass-*` to a content surface, never a cursor spotlight (`.glass-lift` was deliberately not ported).
