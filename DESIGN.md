...
- Horizontal sticky bar at EVERY width, presented as a **dock**: 28 px medallions so all steps fit in one row without scrolling; only the active step keeps its label. Pointer proximity magnifies the step under the cursor (peak ×1.3, radius 96 px, parabolic falloff — macOS Dock fisheye) and unfolds its label + stamp (`grid-template-columns` 0fr→1fr); keyboard focus unfolds the same way; nothing scales under reduced motion. No vertical side panel — the in-content medallion rail is the vertical wayfinding (user decision 2026-09-01 after review: a side rail duplicated the medallions and differed between laptop and monitor). Scroll-spy via IntersectionObserver; clicking a step moves focus to the section `<h2>`.
