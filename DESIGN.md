...
- **Three shells** (mobile pass 2026-09-06 — `docs/research/mobile-synthesis.md` is the binding spec, six research reports behind it in `docs/research/mobile-*.md`). `useViewportClass()` returns `phone | tablet | desktop`; never use `sm:` to mean "phone".
  - **Phone (< md / 768)**: 48 px top bar carrying the page title, an up-link (« ‹ Parent », never labelled "Retour"/"Back"), the page's ONE primary action as a filled 40 px icon button, and a « ⋯ » action sheet — the body paints no H1. Fixed 56 px bottom bar, role-derived by the **combo rule** (`mobileBarFor`): ≤ 4 destinations → all + Profil; ≥ 5 → top 4 + a labelled « Plus » sheet (never a hamburger, never a side drawer). One bottom bar at a time: a page's `<BottomActionBar>` replaces the nav bar, and `--bottom-bar` follows so the scroller, toasts and sheets clear whatever is there. Tables become row lists, dialogs become bottom sheets (multi-field ones full-screen), menus and hover clusters become action sheets, tooltips are off, no FAB, no ⌘K. Tutorial launcher lives in the Plus / Profil sheet.
  - **Tablet (768–1023)**: no bottom bar; the sidebar is a collapsed icon rail; 56 px top bar with the breadcrumb and the workspace-tab strip; centred dialogs; side sheets 420 px.
  - **Desktop (≥ lg / 1024)**: unchanged.
  - Phone type/spacing/target/glass rules live in `src/app/mobile.css`, gated `@media (max-width: 47.9375rem), (max-height: 30rem)` so a landscape phone keeps the phone rhythm: body 15, captions 12, page margin 16, card padding 16, rows 48 / buttons 44–48, blur on the two bars only, no grain. Density zoom is mouse-class only.
...
- **Motion**: animate only `transform`/`opacity`; never `transition-all`; 100–200 ms controls, 200–300 ms panels; exits ≈ ⅔ of enters and fade-only. Three curves (`docs/motion-spec.md` §2, 2026-09-02): `ease-standard` (on-screen movement, recallable-panel exits), `ease-enter` (decelerate, anything appearing), `ease-exit` (accelerate, permanent departures) — plus `ease-soft` for toast entrances only. Everything respects `prefers-reduced-motion` (`motion-reduce:*`; smooth scrolls via `lib/motion.ts scrollBehavior()`; a global rule rests `.animate-spin`). Charts never animate their entrance. Full contracts + frequency classes (F0 never-animate → F3 hero moments): `docs/motion-spec.md`.
...
- **Layout choreography**: every layout change (focus mode, sidebar, panels) animates with the shared `ease-standard` curve (Material standard, cubic-bezier(0.2,0,0,1)) at 300 ms; outgoing content fades out fast (100 ms), tracks animate (`grid-template-columns` / `grid-template-rows` / `max-width` are animatable — use 0fr/0px tracks with `min-w-0 overflow-hidden` wrappers, never `display:none` mid-motion), incoming content fades in over 200 ms with a 100 ms delay (Material fade-through weighting — owner option D2, 2026-09-02). Anything that must animate out stays mounted via `hooks/use-presence.ts`. All of it under `motion-reduce:transition-none`.
