...
- **Lightbox** (`components/document-preview-lightbox.tsx`): the window follows the media's orientation at lg+ — portrait image/PDF (A4 default) → tall window (height leads, width from the natural ratio), landscape → wide window; below lg it stays the bottom-sheet dialog. Pass `hideCloseButton` — the header has its own controls. Zoom: on-canvas − / % / + pill (25 % steps, reset on the %), wheel via `smoothStep` (a few % per notch — never a jump), pinch, double-click +50 %. Multi-page documents: pass `pages` (the slot's files) + `onPageChange` → ‹ › paging, arrow keys, « i / n ».
