  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md flex flex-col">
        <SheetHeader>
          <SheetTitle>{t('Workflow du Dossier:')} {dossier.refExpert}</SheetTitle>
          <SheetDescription>
            {t("Suivi en temps réel de l'état d'avancement.")}
          </SheetDescription>
        </SheetHeader>
        <div className="flex-1 overflow-y-auto py-6 pr-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-sm text-muted-foreground italic">{t('Chargement du workflow...')}</p>
            </div>
          ) : !logs || logs.length === 0 ? (
                ...
                <div key={log.id} className="relative flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      {formatDate(log.date)} {t('par')} <span className="font-semibold text-primary">{log.user}</span>
                    </p>
                  </div>
                </div>
              ))}
