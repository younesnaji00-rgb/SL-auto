...
import { X, Image as ImageIcon, FileText, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ReferencePanel({ dossierId, isOpen, onClose }: ReferencePanelProps) {
  ...
  return (
    <div className="w-1/2 min-w-[300px] border-r bg-card flex flex-col shrink-0">
      {/* Header */}
      ...
      {/* Full-height viewer */}
      <div className="flex-1 overflow-auto bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
        {!selectedId ? (
          ...
          isImageFile(selectedItem?.name || selectedItem?.nom || '') ? (
            <img
              src={viewerUrl}
              alt={selectedItem?.name || 'Référence'}
